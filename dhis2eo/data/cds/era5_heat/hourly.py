...
def download(
    ...
    """
    Retrieves ERA5-HEAT hourly climate data for a given bbox, variables, and start/end dates.
    ...
    # Determine last date for which we can expect ERA5-HEAT to be complete
    # ERA5-HEAT seems to have roughly 6-7 days of lag
    ...
    # Begin downloads
    # NOTE: Although ecmwf-datastores allows asynch jobs, era5-heat is limited to max 1 runnning job
    ...
    for year, month in iter_months(start_year, start_month, end_year, end_month):
        ...
        # As a simple solution, we instead check if the month is expected to be complete (based on the reported publishing lag of ERA5-HEAT)
        ...
        if (year,month) >= (last_updated_date.year, last_updated_date.month):
            logger.warning(
                ...
                f'Latest available date expected in ERA5-HEAT: {last_updated_date.isoformat()}'
            )
            continue
        ...
        if overwrite is False and save_path.exists():
            ...
            # Submit job request and save results to target path
